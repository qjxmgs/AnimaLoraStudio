...
先阅读 [v0.27 升级说明](upgrading-v0.27.md)：停止任务并备份数据，再更新依赖；首次启动会自动拆分旧设置与 LLM 预设存储。

## 下载模型

设置抽屉打开时，临时成功与错误提示仍显示在上层，并保持对辅助技术可见；提示不会移动键盘焦点，抽屉后的页面仍不可交互。
...
3. **预处理** ✱ — 总览（多选 + 一键撤销）+ 去重审核 + 放大（ESRGAN / Real-ESRGAN 多预设）+ 裁剪（手动框选 + 按宽高比预填）+ 涂抹（可选择覆盖原图或绘制训练遮罩，并可运行[自动头部遮罩](auto-head-mask.md)辅助编辑，结果需手动保存）。不需要可直接跳过。
...
「队列」页查看任务，进**任务详情**看日志 / 监控 / 输出（含一键全量 zip 下载）。若详情加载失败，可在错误提示中点**重新加载**，只重新读取当前任务，不会重新运行任务；刷新失败时已显示的详情会保留。数据任务列表加载失败时也可在错误提示中重新加载；刷新失败保留已有任务行，不会将失败显示为“暂无数据任务”。GPU 队列默认只看训练任务，可展开筛选切换类型；进行中/待办与历史分别报告读取错误，可只重新加载失败的部分。空态说明当前任务类型或筛选范围。任务的“结束”时间不代表成功，等待状态不显示推算排位；无有效步数时显示未知进度。时间按界面语言显示，列表省略文本可悬停查看，详情参数保留全文。任务行的详情入口支持键盘访问和在新标签页打开，行内操作按钮独立执行；删除记录和应用快照确认框支持 Tab / Shift+Tab、Esc 返回及焦点恢复，提交期间禁止关闭或重复提交。

设置页和打标页打开同一个 LLM 预设编辑器。打开后焦点进入编辑器，Tab / Shift+Tab 在其中循环；Escape 与“完成”或关闭按钮一样退出，并触发当前字段原有的失焦提交，不会撤销即时保存的修改。删除或恢复内置预设在同一弹窗内确认；取消或 Escape 返回编辑内容与原操作位置，不关闭后面的设置抽屉。确认请求执行期间不能重复提交；失败时可在当前确认视图重试或返回编辑。删除后若原“编辑”入口已消失，焦点回到仍打开的设置抽屉。

全屏图片预览打开后，焦点进入预览画面；可用方向键浏览，Tab / Shift+Tab 在预览画面与关闭、缩放、导航控件间循环，Escape 关闭后返回原入口。聚焦按钮时，Enter / Space 只执行该按钮的动作，不会同时加入或移除图片；筛选页的加入 / 移除快捷键仅在预览画面获得焦点时生效，长按不会重复提交。预览打开期间，Ctrl / Cmd+K 不会另开全局搜索。
...
训完后侧栏 **测试**：跑单图 / XY 矩阵 / 推理 daemon 评测 LoRA，prompt 可从训练集直接拉，不用切 ComfyUI 反复测。LoRA 分区的目录抽屉按项目 / 来源分层浏览项目 checkpoint、Studio 默认目录和自定义目录；项目 / 来源层支持按最近活跃、创建时间或名称排序以及来源筛选，进入目录后支持按最新修改或名称排序，项目版本过滤位于搜索与排序的右侧；额外目录在 **设置 → 测试** 管理。XY 矩阵的 X / Y 轴集中在右侧编辑抽屉，可选择 checkpoint 或 LoRA 强度轴并拖拽调整值顺序。提示词分区的 **从画廊选取** 还能按来源、多选分级、时间范围和 tag 浏览 Danbooru / Gelbooru；tag 搜索复用正向提示词的自动补全（选中后自动转成 Booru 下划线格式），分级选项在收起菜单后统一生效，时间范围则收纳在带红点状态提示的按钮中。筛选条件与浏览页会保存在当前浏览器，也可直接输入页码跳转。选中一张图后可用全局 WD14、CLTagger 或 LLM 设置打标，结果会直接替换“训练集提示词”；打开“自动打标”后，点击生成会先为当前所选图片打标并使用新提示词提交，手动打标本身不会触发生图。画廊抽屉会在提示词、LoRA、参数分页之间保持打开，直到手动收起或打开另一个抽屉。使用前请先在设置页配置对应 Booru 凭据与打标器；远程缩略图由 Studio 限流代理并按图片 ID 缓存，不会自动导入训练集。
