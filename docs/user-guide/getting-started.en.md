...
View tasks on the **Queue** page; open **task detail** for logs / monitoring / output (with one-click full zip download). If details fail to load, use **Reload** in the error notice to fetch the current task again without rerunning it. A failed refresh keeps previously loaded details visible. The data-task list also offers Reload in its error notice: failed refreshes retain loaded rows rather than reporting that there are no tasks. The GPU queue initially shows training tasks only; expand filters to change the type. Active/pending tasks and history report read failures separately, with Reload for the failed source. Empty states identify the current type or filter scope. “Ended” does not imply success, and waiting tasks do not show an inferred queue rank. Progress is indeterminate without valid step counts. Times follow the interface language; hover truncated list text to see it in full, and task parameters remain unabridged in detail. Task-detail links support keyboard access and opening in a new tab, while row action buttons remain independent. Delete-record and apply-snapshot confirmations support Tab / Shift+Tab, Escape and focus return; closing or duplicate submission is blocked while a request is pending.
