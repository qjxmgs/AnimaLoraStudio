...
### Local distribution: provenance-reviewed contours

The qjxmgs level-two distribution retains its previously approved contour review
workflow alongside the upstream direct-edit rectangles. It remains in Retouch's
Training mask tool panel, defaults to face contours, and previews the exact bitmap
before explicit Apply. Its separate Apply / replacement / protected Undo controls
are intentional: replacement must restore a verified earlier manual baseline,
which the transient editor history cannot reconstruct. This is not a new general
save pattern. The upstream header Auto mask action and grouped unsaved edits stay
unchanged. Contour jobs must never be incorporated there as rectangles. The two
flows share busy/unsaved gates and cannot concurrently write masks. Switching
brush modes keeps a started contour review mounted. Legacy compact controls remain
a compatibility surface; this merge does not redesign the review panel.
